...
function getRecibos(){
    ...
    ajax("http://localhost/cobro3/cobdes_importe_dinamico/", "POST", "txt", setRecibos, formData);
}
...
function submit(event){
    ...
    ajax("../cobdes_importe_dinamico/", "POST", "txt", setRecibos, formData);
}
...
function botonesCobrarClick(){
    for(let x = 0; x < botonesCobrar.length; x++){
        botonesCobrar[x].addEventListener("click", function() {
            ...
            ajax("../cobdes_importe_dinamico/", "POST", "nada", getRecibos, formData);
        });
    }
}
